...
public:
    ...
    std::unique_ptr<ExprAST> LogError(const char* str) {
        fprintf(stderr, "Error: %s. Current Token: '%s'\n", str, curTok.text.c_str());
        return nullptr;
    }
    ...
    std::unique_ptr<ExprAST> ParseBinOpRHS(int ExprPrec, std::unique_ptr<ExprAST> LHS) {
        while (true) {
            ...
            char BinOp = curTok.text[0]; // Simplification: assume 1-char ops for now
    ...
    std::unique_ptr<ExprAST> ParseVarDecl() {
        ...
        if (curTok.type == TokenType::Semicolon) getNextToken(); // eat ';'
    ...
    bool isEOF() { return curTok.type == TokenType::EoF; }
};
